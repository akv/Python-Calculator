...
class calculator:
 def getandreplace(self):
  ...
  self.newtext=self.expression.replace(self.divSign,'/')
 ...
 def __init__(self,master):
  """Constructor method"""
  master.title('Arun\'s Calculator') 
  master.geometry("320x200+30+30")
  master.resizable(0, 0) 
  img = PhotoImage(file='calc.png')
  master.call('wm', 'iconphoto', root._w, img)

  self.e = Entry(master)
  self.e.grid(row=0,column=0,columnspan=6,pady=4)
  self.e.focus_set() #Sets focus on the input text area  
  self.div='÷'
  self.divSign=self.div.decode('utf-8')
  ...
  Button(master,text="÷",width=3,command=lambda:self.action(self.divSign)).grid(row=1, column=3) 
...
#obj.pack(fill=None, expand=False)
root.mainloop()
